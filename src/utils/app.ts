...
//     ip = ip.split(',')[0]
//   }
//   ip && ($ky.defaults.headers.common['x-forwarded-for'] = ip as string)

//   $ky.defaults.headers.common[
//     'User-Agent'
...
//       ?.split('=')[1]
//     if (token) {
//       $ky.defaults.headers['Authorization'] = `bearer ${token.replace(
//         /^Bearer\s/i,
//         '',
//       )}`
//     }
...
export async function fetchInitialData(): Promise<InitialDataType> {
  ...
  const [aggregateDataState, configSnippetState] = await Promise.allSettled([
    ...
    apiClient.snippet.getByReferenceAndName<KamiConfig>(
      'theme',
      import.meta.env.NEXT_PUBLIC_SNIPPET_NAME || 'kami',
    ),
  ])
